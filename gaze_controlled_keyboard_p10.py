...
# loads sound left right and blink
...
# drawn menu of left and right keyboard
def draw_menu():
...
# calculate the ratio between width and height of our eyes
def midpoint(p1, p2):
...
# find out when we have eyes open or closed
...
# draw contour around eyes Red when eyes open and Green when eyes closed
...
# when we want to select menu left or right
...
# counters
frames = 0
letter_index = 0  # each time we want to light up we increment this index
blinking_frames = 0
frames_to_blink = 6  # if we kept the eyes closed for atleast 6 frames then it is considered as a word
frames_active_letter = 9  # to light up a new letter after 9 letters

# Text and keyboard settings
text = ""  # to make string word
keyboard_selected = "left"  # for keyboard selection
